...
class MultiSingleStreamBlockLoraProcessor(nn.Module):
    def __init__(self, in_features: int, out_features: int, ranks=[], lora_weights=[], network_alphas=[], device=None, dtype=None, cond_width=512, cond_height=512, n_loras=1):
        super().__init__()
        ...
        self.q_loras = nn.ModuleList([
            LoRALinearLayer(in_features, out_features, ranks[i],network_alphas[i], device=device, dtype=dtype, cond_width=cond_width, cond_height=cond_height, number=i, n_loras=n_loras)
...
class MultiDoubleStreamBlockLoraProcessor(nn.Module):
    ...
    def __call__(self,
        ...
        cond_start = block_size - scaled_cond_size * self.n_loras
        ...
        if cache or not self.use_bank:
            ...
            if self.use_bank:
                self.bank_kv.append(key[..., cond_start:, :])
                self.bank_kv.append(value[..., cond_start:, :])
            ...
            num_cond_blocks = self.n_loras
            mask = torch.ones((scaled_seq_len, scaled_seq_len), device=hidden_states.device)
            mask[ :scaled_block_size-scaled_cond_size, :] = 0  # First block_size row
            for i in range(num_cond_blocks):
                start = i * scaled_cond_size + scaled_block_size-scaled_cond_size
                end = (i + 1) * scaled_cond_size + scaled_block_size-scaled_cond_size
                mask[start:end, start:end] = 0  # Diagonal blocks
            mask = mask * -1e20
            mask = mask.to(query.dtype)
